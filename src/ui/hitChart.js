...
export function drawHitChart(canvas) {
  ...
  ctx.textAlign = 'left';  ctx.fillText('-' + rangeMs + ' ms early', 4, H - 12);
  ctx.textAlign = 'right'; ctx.fillText('+' + rangeMs + ' ms late', W - 4, H - 12);
  ...
  if (hitOffsets.length >= 3) {
    ...
    const hint = median > 15 ? '→ ты стабильно ЛЕЙТ, попробуй audioOffset +' + Math.round(median) + ' ms'
               : median < -15 ? '→ ты стабильно ЭРЛИ, попробуй audioOffset ' + Math.round(median) + ' ms'
               : '✓ хорошо откалибровано';
    ctx.fillStyle = '#aee9ff';
    ctx.font = 'bold 11px "JetBrains Mono", monospace';
    ctx.textAlign = 'center';
    ctx.fillText('median ' + median.toFixed(0) + ' ms · σ ' + std.toFixed(0) + ' ms · ' + hint, W / 2, 4);
  }
}
